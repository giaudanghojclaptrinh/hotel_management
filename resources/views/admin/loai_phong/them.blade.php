...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-serif font-bold text-white">Thông tin hạng phòng</h1>
    ...
    <div class="bg-gray-900 rounded-2xl shadow-lg border border-gray-800 overflow-hidden">
        ...
        <div class="bg-gray-800/50 px-8 py-4 border-b border-gray-800">
            <h3 class="text-sm font-bold text-brand-gold uppercase tracking-wider">Chi tiết phòng</h3>
        </div>
            ...
            <form action="{{ route('admin.loai-phong.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="ten_loai_phong" value="{{ old('ten_loai_phong') }}" required placeholder="Ví dụ: Deluxe King Ocean View"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" name="gia" value="{{ old('gia') }}" required placeholder="0"
                                       class="w-full rounded-lg bg-gray-800 border-gray-700 text-white shadow-sm focus:border-brand-gold focus:ring-brand-gold h-11 pr-12 transition-all font-mono font-bold">
                        ...
                        <div>
                            ...
                            <textarea name="mo_ta" rows="6" 
                                      ...
                                      placeholder="Mô tả về tiện nghi, hướng nhìn, diện tích...">{{ old('mo_ta') }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3 bg-gray-800/30 p-4 rounded-xl border border-gray-800">
                                @foreach($tienNghis as $tn)
                                    <label class="flex items-center space-x-2 cursor-pointer group select-none">
                                        <input type="checkbox" name="tien_nghi[]" value="{{ $tn->id }}" 
                                               class="rounded bg-gray-700 border-gray-600 text-brand-gold focus:ring-brand-gold focus:ring-offset-gray-900 transition-all" {{ in_array($tn->id, old('tien_nghi', [])) ? 'checked' : '' }}>
                                        <span class="text-sm text-gray-300 group-hover:text-white transition-colors">{{ $tn->ten_tien_nghi }} <small class="text-gray-500">({{ $tn->ma_tien_nghi }})</small></span>
                                    </label>
                                @endforeach
                            </div>
                    ...
                    <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="bg-blue-900/20 p-4 rounded-xl border border-blue-800/50">
                            ...
                            <p class="text-xs text-blue-300 leading-relaxed">Nên sử dụng hình ảnh tỉ lệ 4:3 hoặc 16:9 để hiển thị đẹp nhất trên website.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 pt-6 border-t border-gray-800 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2.5 bg-brand-gold text-gray-900 rounded-lg font-bold hover:bg-white shadow-md transition-all flex items-center">
                        <i class="fa-solid fa-save mr-2"></i> Lưu Hạng Phòng
                    </button>
                </div>
            </form>
        </div>
